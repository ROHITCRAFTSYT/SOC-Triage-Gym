...
**Event:** BLR5 CCCL × SurrealDB — Memory, Context & Agents, Bengaluru
...
> A real SOC has three tiers. Tier-1 triages alerts. Tier-2 contains confirmed
> threats. A manager audits both. Between them: tickets, escalation paths, SLA
...
> rules. You cannot disable a user without an open P1 ticket. Sound familiar?
>
...
> twenty-seven optimizer steps on a free Colab T4 moves nothing. The pipeline is
...
## 6:50 – 7:50 · WHY THIS FITS TONIGHT (~140 words)

*Scroll to the Memory / Context / Agents band.*

> This meetup is about memory, context, and agents — and this project turned out
> to be a case study in all three.
>
> **Memory:** episode state, the ticket bus, policy history — explicit and
> queryable. An agent that forgets which policy version was active at step twelve
> *loses reward* for it. And if you're wondering where a database like SurrealDB
> fits — tickets are documents, alert correlation is a graph, policy drift is
> time-versioned records. That's the natural next backend.
>
> **Context:** context here isn't static — it *drifts.* Rules change mid-episode.
> NPCs interrupt. Horizons run to two hundred fifty steps. Agents have to triage
> their context the way analysts triage alerts.
>
> **Agents:** not one — a team of three, plus an adversary underneath that
...
> Eleven years ago, the right alert died in a handoff between this city and
> Minneapolis.
>
...
Absorbs every overrun. If you're on time, you get 3–4 questions — answers are
pre-written in [TALK_PLAN.md §7](TALK_PLAN.md). The SurrealDB question is
near-guaranteed; you already planted the answer in the Memory paragraph.
